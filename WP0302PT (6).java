...
public class WP0302PT extends bTransaction{
	...
	public boolean Query()throws Throwable {
		sa.append("查詢\n");
		String idkey=getQueryValue("idkey").trim();	//取得idkey
		sql="select rcvid,meid from wptlproc where idkey='"+idkey+"'";	//取得rcvid
...
//				backupWptmapply(meid,idkey);//備份主檔,商標,描述性說明，使用json
				backupWptmapplyGoodclass(meid,idkey);//json   1.備份主檔,商標,描述性說明  2.備份goodclass
			}
	...
	*/	
	public boolean Delete()throws Throwable{
		...
		for (int i=0 ;i<table1.length ;i++ ){
			...
			revertMemanid(meid,idkey);//回復代理人
	...
	*/
	public void selectTable1(String rcvid)throws Throwable {
		sa.append("搜尋table1表格\n");
		//表格設定
		sql="select distinct '','a'=case when d.idkey is null then '否' else '是' end,a.meid,"
           +"b.regid,b.markname,b.extedead,a.appydate,a.dbbouns,a.rcvno,a.idkey,'修改','',a.rcvid,d.keyno,'','1' as bb"
           +" from wptlproc a"
           ...
           +" union "
           +"select distinct '','a'=case when d.idkey is null then '否' else '是' end,a.meid,"
           +"b.regid,b.markname,b.extedead,a.appydate,a.dbbouns,a.rcvno,a.idkey,'修改','',a.rcvid,d.keyno,'','2' as bb"
           +" from wptlproc a"
           +" left outer join wptmapply b on a.meid=b.meid"
           +" left outer join wptlapbale c on a.meid=c.meid and delmk=0"
           +" left outer join wptlproexte d on a.idkey=d.idkey"
	...
	*/
	public void selectAnnex(String idkey)throws Throwable{		
		sa.append("搜尋-附件\n");		
		sql="select '含件'=case when l.annexid is null then '否' else '是' end ,c.annex_name,isnull(l.description,''),"
	...
	*/
	...
	public String getKeyno()throws Throwable {
		...
		if(keynoStr.length()==10){
			...
			sql = "select top 1 keyno from wptlproexte where left(keyno,2)='10' and  len(keyno)='10' order by keyno desc";                //取得keyno處理
...
//				System.out.println(pcStr);
	...
	*/
	...
	public String getGcKey()throws Throwable {
		talk t=getTalk("TradeMark");
		if(gckey==null) {
			sql = "select max(gckey) from wptlproc_wrkman";                //取得gckey處理
			String[][]ret = t.queryFromPool(sql);
			sa.append(sql+"\n");
...
*/
	...
	*/
	...
	public void saveWptlappyOriApman(String meid)throws Throwable {
		sa.append("備份wptlappyoriapre代表人\n");
		vt=new Vector();
		//搜尋代表人資料
...
//	}	
	...
	*/
	public void backupWptmapplyGoodclass(String meid,String idkey)throws Throwable  {
		...
		jsonObject.put("wptlapbale",jsonArray(sql));//商品類別檔
		sql="SELECT markname,picchk,marktype,markmeid,markappyid,markregid,relchk,exteappydate,cdesctext,desctext "
			+" FROM wptmapply"
			+" WHERE meid ='"+meid+"'";
	
		jsonObject.put("wptmapply",jsonArray(sql));//主檔
